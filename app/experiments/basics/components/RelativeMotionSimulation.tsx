...
export const RelativeMotionSimulation: React.FC<
  RelativeMotionSimulationProps
> = ({
  ballPosition,
  trainPosition,
  trajectoryPoints,
  ...
  return (
    <View style={styles.container}>
      <Svg width={CANVAS_WIDTH} height={CANVAS_HEIGHT} style={styles.svg}>
        ...
        {/* Trajectory */}
        {showTrajectory && adjustedTrajectoryPoints.length > 1 && (
          <>
            <Polyline
              points={adjustedTrajectoryPoints
                .map((p) => `${p.x},${p.y}`)
                .join(' ')}
              fill="none"
        ...
        {ballPosition.y > 0 && !isGrounded && (
          <>
            <Line
              x1={displayBallPos.x}
              y1={displayBallPos.y}
              x2={displayBallPos.x + (isGroundView ? trainSpeed + 8 : 8) * 3}
              y2={displayBallPos.y - Math.max(3, 12 - time * 8) * 2}
              stroke="#ec4899"
              strokeWidth="3"
            />
            {/* Arrow head */}
            <Polygon
              points={`${
                displayBallPos.x + (isGroundView ? trainSpeed + 8 : 8) * 3
              },${displayBallPos.y - Math.max(3, 12 - time * 8) * 2} ${
                displayBallPos.x + (isGroundView ? trainSpeed + 8 : 8) * 3 - 6
              },${displayBallPos.y - Math.max(3, 12 - time * 8) * 2 - 3} ${
                displayBallPos.x + (isGroundView ? trainSpeed + 8 : 8) * 3 - 6
              },${displayBallPos.y - Math.max(3, 12 - time * 8) * 2 + 3}`}
              fill="#ec4899"
            />

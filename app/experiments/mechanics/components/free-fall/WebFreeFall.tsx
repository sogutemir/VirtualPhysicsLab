...
export const WebFreeFall = forwardRef<any, FreeFallProps>(
  (
    { width = window.innerWidth, height = window.innerHeight, onStateChange },
    ref
  ) => {
    const { t } = useLanguage();
    ...
    const canvasWidth = Math.min(width * 0.95, 1000);
    const canvasHeight = Math.min(height * 0.7, 600);
      ...
      (ctx: CanvasRenderingContext2D) => {
        ...
        for (let x = 0; x <= MAX_X; x += 250) {
          ...
          ctx.fillText(
            x.toString() + ' m',
            canvasX,
            canvasHeight - CANVAS_PADDING + 18
          );
        }
        ...
        ctx.textAlign = 'center';
        ctx.fillText(`${t('x', 'x')} (m)`, canvasWidth / 2, canvasHeight - 5);

        ctx.save();
        ctx.translate(15, canvasHeight / 2);
        ctx.rotate(-Math.PI / 2);
        ctx.fillText(`${t('y', 'y')} (m)`, 0, 0);
        ctx.restore();
      },
      [canvasWidth, canvasHeight, toCanvasX, toCanvasY, t]
    );

...
// Memoized Coordinate System Component
const CoordinateSystem = memo<{
  width: number;
  height: number;
}>(({ width, height }) => {
  const elements = [];
  ...
  for (let x = 0; x <= width; x += xStep) {
    const label = Math.round(x - width / 2);
    elements.push(
      <SvgText
        key={`x-label-${x}`}
        x={x}
  ...
  for (let y = 0; y <= height; y += yStep) {
    const label = Math.round(height / 2 - y);
    elements.push(
      <SvgText
        key={`y-label-${y}`}
        x={centerX - 10}
...
// Memoized Physics Panel Component
const PhysicsPanel = memo<{
  projectiles: Projectile[];
  collisionData?: {
    hasCollided: boolean;
    impulse: number;
...
}>(({ projectiles, collisionData, t }) => {
  const physicalValues = useMemo(() => {
  ...
  }, [projectiles, collisionData]);

  if (!physicalValues) return null;
  ...
  return (
    <G x="20" y="20">
      ...
      <SvgText x="10" y="45" fill="#374151" fontSize="11">
        {t('Etki Eden Kuvvet:', 'Applied Force:')} {physicalValues.force} N
      </SvgText>
      <SvgText x="10" y="65" fill="#374151" fontSize="11">
        {t('Momentum:', 'Momentum:')} {physicalValues.momentum} kg⋅m/s
      </SvgText>
      <SvgText x="10" y="85" fill="#374151" fontSize="11">
        {t('Kinetik Enerji:', 'Kinetic Energy:')} {physicalValues.kineticEnergy}{' '}
        J
      </SvgText>
      <SvgText x="10" y="105" fill="#374151" fontSize="11">
        {t('Hız:', 'Velocity:')} {physicalValues.velocity} m/s
      </SvgText>
    </G>
  );
});

// Memoized Target Box Component
const TargetBoxVisual = memo<{
  targetBox: TargetBox;
  t: (tr: string, en: string) => string;
}>(({ targetBox, t }) => {
...
// Memoized Projectiles Component
const ProjectilesVisual = memo<{
  projectiles: Projectile[];
}>(({ projectiles }) => {
  return (
    <>
...
// Memoized Stuck Projectiles Component
const StuckProjectilesVisual = memo<{
  projectiles: Projectile[];
  t: (tr: string, en: string) => string;
}>(({ projectiles, t }) => {
  return (
...
// Memoized Penetration Effects Component
const PenetrationEffects = memo<{
  projectiles: Projectile[];
  targetBox: TargetBox;
  t: (tr: string, en: string) => string;
}>(({ projectiles, targetBox, t }) => {
  return (
    <>
      {projectiles.map((projectile, index) => {
        if (
...
// Memoized Velocity Vectors Component
const VelocityVectors = memo<{
  projectiles: Projectile[];
  targetBox: TargetBox;
  isRunning: boolean;
}>(({ projectiles, targetBox, isRunning }) => {
  ...
  projectiles.forEach((projectile, index) => {
    if (projectile.stuckInside) return;
...
const SimulationCanvas = memo<SimulationCanvasProps>(
  ({ width, height, projectiles, targetBox, isRunning, collisionData }) => {
    const { t } = useLanguage();
    ...
    return (
      ...
      >
        <Svg width={canvasSize.width} height={canvasSize.height}>
          ...
          <GridLines width={canvasSize.width} height={canvasSize.height} />
          <CoordinateSystem
          ...
          {/* Hedef kutu */}
          <TargetBoxVisual targetBox={targetBox} t={t} />

          {/* Mermiler */}
          <ProjectilesVisual projectiles={projectiles} />

          {/* Saplanmış mermiler */}
          <StuckProjectilesVisual projectiles={projectiles} t={t} />

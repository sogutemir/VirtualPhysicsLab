...
interface AtwooodMachineSystemProps {
  state: AtwooodMachineState;
}

export const AtwooodMachineSystem: React.FC<AtwooodMachineSystemProps> = ({
  state,
}) => {
  const svgWidth = 500;
  const svgHeight = 650;

  // Physics to visual conversion
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Pulley center */}
        <Circle cx={pulleyCenterX} cy={pulleyCenter} r={8} fill="#6b7280" />

        {/* Pulley center dot */}
        <Circle cx={pulleyCenterX} cy={pulleyCenter} r={3} fill="#374151" />
        ...
        {/* Mass 1 highlight */}
        <Circle
          cx={mass1X - mass1Size / 3}
          cy={mass1Y - mass1Size / 3}
          r={mass1Size / 4}
        ...
        {/* Mass 2 highlight */}
        <Circle
          cx={mass2X - mass2Size / 3}
          cy={mass2Y - mass2Size / 3}
          r={mass2Size / 4}
          fill="rgba(255, 255, 255, 0.5)"
        />
        ...
        {state.velocity !== 0 && (
          <G>
            ...
            <Line
              ...
              y2={mass1Y + (state.velocity > 0 ? 25 : -25)}
              stroke={state.velocity > 0 ? '#ef4444' : '#3b82f6'}
              strokeWidth={2}
            />
            ...
            <Line
              ...
              y2={mass2Y + (state.velocity > 0 ? -25 : 25)}
              stroke={state.velocity > 0 ? '#3b82f6' : '#ef4444'}
              strokeWidth={2}
            />
          </G>
        )}
        ...
        <Line
          x1={50}
          y1={pulleyCenter + initialOffset}
          x2={svgWidth - 50}
          y2={pulleyCenter + initialOffset}
          stroke="#10b981"
          strokeWidth={1}
          strokeDasharray="5,5"
          opacity={0.5}
      ...
      <View style={styles.debugInfo}>
        ...
        <Text style={styles.debugText}>v: {state.velocity.toFixed(3)} m/s</Text>
        <Text style={styles.debugText}>x₁: {state.position1.toFixed(3)}m</Text>
        <Text style={styles.debugText}>x₂: {state.position2.toFixed(3)}m</Text>
      </View>
    </View>
  );
};
